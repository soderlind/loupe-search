...
class WpLoupeFieldManager {
	...
	async startBatchedReindex() {
		...
		this.showProgress( 0, this.__( 'Starting…', 'wp-loupe' ) );
		...
		try {
			...
			while ( true ) {
				if ( this.cancelRequested ) {
					this.showStatusMessage(
						this.__( 'Reindex cancelled.', 'wp-loupe' ),
						'info'
					);
					break;
				}
				...
				if ( response && response.done ) {
					this.showProgress( 100, this.__( 'Complete', 'wp-loupe' ) );
					this.showStatusMessage(
						this.__(
	...
	renderIndexHealth( items ) {
		...
		const reasons = {
			index_missing: this.__( 'Not indexed', 'wp-loupe' ),
			index_needs_reindex: this.__( 'Needs reindex', 'wp-loupe' ),
			index_unreadable: this.__( 'Index unreadable', 'wp-loupe' ),
			unknown_post_type: this.__( 'Unknown post type', 'wp-loupe' ),
			invalid_post_type: this.__( 'Invalid post type', 'wp-loupe' ),
		};
		...
		thead.innerHTML = `<tr><th>${ this.__(
			'Post type',
			'wp-loupe'
		) }</th><th>${ this.__( 'Status', 'wp-loupe' ) }</th><th>${ this.__(
			'Published',
			'wp-loupe'
		) }</th></tr>`;
		...
		items.forEach( ( item ) => {
			...
			if ( item.ready ) {
				...
				badge.textContent = this.__( 'Ready', 'wp-loupe' );
			} else {
				badge.classList.add( 'is-warning' );
				badge.textContent =
					reasons[ item.reason ] ||
					this.__( 'Needs reindex', 'wp-loupe' );
			}
