...
class WP_Loupe_Loader {
	...
	private function load_dependencies() {
		...
		// Prefixed (scoped) third-party libraries, generated by Strauss at build time.
		// Isolates Loupe and its Symfony/Doctrine/PSR dependencies from other plugins.
		if ( file_exists( LOUPE_SEARCH_PATH . 'vendor-prefixed/autoload.php' ) ) {
			require_once LOUPE_SEARCH_PATH . 'vendor-prefixed/autoload.php';
		} else {
			// Dev checkout with no Strauss build: alias the unscoped Loupe classes
			// to the prefixed names the source references so the plugin still loads.
			self::alias_unscoped_loupe_classes();
		}
	...
	/**
	 * Alias unscoped Loupe classes to their Strauss-prefixed names.
	 *
	 * The plugin source references Loupe under the prefixed namespace produced by
	 * the build-time Strauss step. In a raw dev checkout (no build) only the
	 * original `Loupe\Loupe\*` classes exist, so map them onto the prefixed names
	 * the code expects. A real build defines the prefixed classes for real, so
	 * this runs only when `vendor-prefixed/` is absent.
	 *
	 * @return void
	 */
	private static function alias_unscoped_loupe_classes() {
		$classes = array(
		...
		foreach ( $classes as $class ) {
			$prefixed = 'Soderlind\\Plugin\\LoupeSearch\\Vendor\\' . $class;
			if ( ! class_exists( $prefixed, false ) && class_exists( $class ) ) {
				class_alias( $class, $prefixed );
			}
		}
	}
