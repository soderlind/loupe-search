...
### Changed
- Updated the plugin screenshots to reflect the current settings UI.
- Corrected the screenshot descriptions in `readme.txt` to match the current Search Behavior tab (tokenization, prefix search, typo tolerance).

### Internal
- Added unit tests for the indexer, the plugin loader and the deprecated `WP_Loupe_Search` facade, and strengthened the existing hook, abilities and REST tests.
...
### Fixed
- Front-end search now works when a public, searchable post type (e.g. attachments) is not indexed. Previously the plugin bailed out and fell back to WordPress's default search, returning no Loupe results.
...
[1.2.4]: https://github.com/soderlind/loupe-search/releases/tag/1.2.4
[1.2.3]: https://github.com/soderlind/loupe-search/releases/tag/1.2.3
...
[1.0.0]: https://github.com/soderlind/loupe-search/releases/tag/1.0.0
